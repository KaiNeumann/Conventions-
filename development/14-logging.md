...
Logs answer "what happened, when, to what?" â€” for humans scanning a
...
1. All runtime logs live under **`logs/`** in the app's data or profile
   directory â€” never scattered next to code, never in temp dirs.
...
2. **Level**: `DEBUG` / `INFO` / `WARN` / `ERROR` from a fixed
   vocabulary.
3. **Identifiers**: each entry names what it concerns â€” app/component
   plus the relevant entity id (`task=42`, `hoster=rapidshare`) â€” used
...
1. `DEBUG` is off by default and toggled via settings â€” never needed to
...
3. No secrets, tokens, or personal data in any level â€” placeholders
...
1. GUIs surface state in the interface (statusbar/toast) and write the
   detail to the log â€” the log is the deep layer, not the primary UX.
2. Agents debugging read **log tails** (`--tail` / last N lines) and
   grep by identifier â€” never cat whole files into context.
